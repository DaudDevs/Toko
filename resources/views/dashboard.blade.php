...
<head>
  ...
  <title>Dashboard Member - TokoKita</title>
...
<main class="flex-1 p-4 sm:p-6 md:p-10 ml-16 md:ml-0">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4" data-aos="fade-down" data-aos-delay="100">
      ...
      <a href="{{ route ('product')}}" class="bg-yellow-400 hover:bg-yellow-500 text-gray-900 font-semibold px-5 py-2 rounded-lg transition shadow-lg">
        Lihat Semua Produk
      </a>
    </div>

    <h1 class="text-3xl font-bold mb-4" data-aos="fade-down" data-aos-delay="200">Produk Kamu</h1>
...
<div id="produk-container" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6" data-aos="fade-up" data-aos-delay="300">
  @forelse ($products as $product)
    ...
    <div class="bg-white/10 p-4 rounded-xl shadow-lg glow transition"
      ...
      <img src="{{ asset('storage/' . $product->image) }}" class="rounded mb-3 w-full h-40 object-cover" alt="{{ $product->name }}">
      ...
      <p class="text-sm mb-2">
        Stok: 
        <span class="{{ $stockCount > 0 ? 'text-green-400' : 'text-red-400' }}">
          {{ $stockCount }}
        </span>
      </p>
      ...
      <div class="flex justify-between items-center">
        ...
        @else
          <button class="bg-gray-500 cursor-not-allowed px-3 py-1 rounded text-sm opacity-50" disabled>
            Habis
          </button>
        @endif
      </div>
  ...
  @empty
    <p class="text-white/70">Belum ada produk.</p>
  @endforelse
</div>
